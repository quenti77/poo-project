...
class MultiSelect extends AbstractInput
{
    ...
    private function askWithKeyboard(): array
    {
        ...
        $this->output->comment("(Utilisez ↑↓ pour naviguer, Espace pour sélectionner, Entrée pour valider)");
        $this->output->writeln();
        ...
        try {
            while (true) {
                ...
                if ($key === Cursor::KEY_UP) {
                    ...
                    $this->clearOptions(count($keys));
                } elseif ($key === Cursor::KEY_SPACE || $key === ' ') {
                    // Toggle selection
                    ...
                    if (empty($selected) && $this->required) {
                        ...
                        $this->clearOptions(count($keys));
                        $this->output->error("Vous devez sélectionner au moins une option.");
                        $this->output->terminal->hideCursor();
                        $this->output->terminal->enableRawMode();
                        $this->displayOptionsWithHighlight($highlightedIndex, $selected);
                        continue;
            ...
            $this->output->success('Sélectionné: ' . implode(', ', $selectedValues));
    ...
    private function askWithInput(): array
    {
        ...
        $this->displayQuestion();
        $this->output->writeln();
        $this->output->comment("Entrez les numéros séparés par des virgules (ex: 1,3,5)");
        $this->output->comment("Ou tapez 'all' pour tout sélectionner, 'none' pour tout désélectionner");
        $this->output->writeln();
        ...
        while (true) {
            ...
            if (strtolower($input) === 'all') {
                ...
                $this->output->success("Toutes les options sélectionnées!");
                continue;
            }

            if (strtolower($input) === 'none') {
                $selected = [];
                $this->output->warning("Toutes les options désélectionnées.");
                continue;
            }
                ...
                if ($numericInput !== false) {
                    ...
                    if (isset($keys[$index])) {
                        ...
                        if (in_array($key, $selected, true)) {
                            ...
                            $this->output->warning("Désélectionné: {$this->options[$key]}");
                        } else {
                            $selected[] = $key;
                            $this->output->success("Sélectionné: {$this->options[$key]}");
                        }
                    }
                    ...
                    if (in_array($num, $selected, true)) {
                        ...
                        $this->output->warning("Désélectionné: {$this->options[$num]}");
                    } else {
                        $selected[] = $num;
                        $this->output->success("Sélectionné: {$this->options[$num]}");
                    }
                }
            }
