...
class CoreFilters
{
    public static function register(FilterRegistry $registry): void
    {
        ...
        $registry->register('replace', static fn($v, string $search, string $replace) =>
            str_replace($search, $replace, (string) $v)
        );

        $registry->register('split', static fn($v, string $delimiter = '') =>
            $delimiter === '' ? mb_str_split((string) $v) : explode($delimiter, (string) $v)
        );

        $registry->register('nl2br', static fn($v) =>
        ...
        // Numbers
        $registry->register('number_format', static fn($v, int $decimals = 0, string $dec = ',', string $thousands = ' ') =>
            number_format((float) $v, $decimals, $dec, $thousands)
        );
        ...
        $registry->register('slice', static fn($v, int $start, ?int $length = null) =>
            is_array($v) ? array_slice($v, $start, $length) : mb_substr((string) $v, $start, $length)
        );
        ...
        $registry->register('map', static fn($v, string $key) =>
            is_array($v) ? array_map(static fn($item) => $item[$key] ?? null, $v) : $v
        );

        // Dates
        $registry->register('date', static fn($v, string $format = 'd/m/Y') =>
        ...
        $registry->register('date_modify', static function ($v, string $modify) {
            ...
            $date = new DateTime(is_numeric($v) ? "@$v" : (string) $v);
        ...
        $registry->register('json', static fn($v, int $flags = 0) =>
            json_encode($v, JSON_UNESCAPED_UNICODE | $flags)
        );
        ...
        // Debug
        $registry->register('dump', static fn($v) => '<pre>' . htmlspecialchars(print_r($v, true)) . '</pre>');
